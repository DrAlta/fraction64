...
impl TryFrom<f32> for Fraction {
    ...
    fn try_from(value: f32) -> Result<Self, Self::Error> {
        if value.is_nan() {
            return Err("Fraction doesn't have NaN".into());
        };
        if value.is_infinite() {
            return Err("Fraction doesn't have infinities".into());
        };
        ...
        Ok(
            float_to_fraction_direct_from_mantissa_and_exponent(value + 0.0)
                .ok_or("Float is out of range".to_string())?,
        )
    }
}
...
fn float_to_fraction_direct_from_mantissa_and_exponent<F: Float>(value: F) -> Option<Fraction> {
    ...
    let Some(x) = 2_u128.checked_pow(exponent.unsigned_abs() as u32) else {
        return Some(if exponent > 0 {
            Fraction {
                sign: new_sign,
                numer: std::u64::MAX,
                denom: std::num::NonZeroU64::new(1).unwrap(),
            }
        } else {
            Fraction::ZERO
        });
    ...
    let (radix_numer, radix_denom) = if exponent > 0 { (1, x) } else { (x, 1) };
    let mantissa = mantissa as u128;
    Some(u128_to_fraction(
        new_sign,
        mantissa * radix_denom,
        radix_numer,
    ))
}
...
mod tests {
    ...
    fn point25_f32() {
        ...
        assert_eq!(value, Fraction::new(1, NonZeroU64::new(4_u64).unwrap()));
    }
    #[test]
    fn point25_f64() {
    ...
    fn neg_point25_f64() {
        ...
        assert_eq!(value, Fraction::new_neg(1, NonZeroU64::new(4_u64).unwrap()),);
    }
}
