...
impl Fraction {
    pub fn powi(&self, p: i32) -> Self {
        ...
        let n = (self.numer as u128)
            .checked_pow(p.unsigned_abs())
            .expect("the numen over flowed");
        let d = (self.denom.get() as u128)
            .checked_pow(p.unsigned_abs())
            .expect("the demon over flowed");

        let (numer, denom) = if p < 0 { (d, n) } else { (n, d) };
