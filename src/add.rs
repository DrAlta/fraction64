            ...
            let (new_sign, new_numer) = match (&self.sign, &other.sign) {
                ...
                (Sign::Negative, Sign::Negative) => {
                    let Some(thing) = self.numer.checked_add(other.numer) else {
                        ...
                        return Fraction { sign: Sign::Negative, numer, denom: NonZeroU64::new(denom).unwrap() };
                    };
                    (Sign::Negative, thing)
                },
            };
            ...
            match (self.denom.checked_mul(other.denom), self.numer.checked_mul(other.denom.get()), other.numer.checked_mul(self.denom.get())) {
                (Some(common_denom), Some(new_numer_self), Some(new_numer_other)) => {
                    let (new_sign, new_numer) = match (&self.sign, other.sign.clone()) {
                        (Sign::Positive, Sign::Positive) => (Sign::Positive, new_numer_self + new_numer_other),
                        (Sign::Positive, Sign::Negative) => {
                        ...
                        (Sign::Negative, Sign::Negative) => (Sign::Negative, new_numer_self + new_numer_other),
                    };
...
mod tests {
    ...
    fn neg_one_plus_third() {
        assert_eq!( Fraction::NEG_ONE + Fraction::THIRD, Fraction::NEG_TWO_THIRDS);
    }
    #[test]
    fn neg_one_plus_neg_third() {
        assert_eq!( Fraction::NEG_ONE + Fraction::NEG_THIRD, Fraction::NEG_FOUR_THIRDS);
    }
    #[test]
    ...
    fn neg_third_plus_neg_one() {
        assert_eq!( Fraction::NEG_THIRD + Fraction::NEG_ONE, Fraction::NEG_FOUR_THIRDS)
    }
}